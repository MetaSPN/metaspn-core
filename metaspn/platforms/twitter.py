...
class TwitterPlatform(BasePlatform):
    ...
    def compute_metrics(self, activities: list["Activity"]) -> dict[str, Any]:
        ...
        original = sum(
            1
            for a in activities
            if not a.raw_data.get("is_reply") and not a.raw_data.get("is_retweet")
        )
        ...
        threads = len(
            set(a.raw_data.get("thread_id") for a in activities if a.raw_data.get("is_thread"))
        )
    ...
    def reconstruct_threads(
        ...
        for thread_id in threads:
            threads[thread_id].sort(key=lambda a: a.raw_data.get("thread_position", 0))
