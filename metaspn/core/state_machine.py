...
class LifecycleState:
    ...
    def from_dict(cls, data: dict) -> "LifecycleState":
        ...
        return cls(
            ...
            phase_entered=datetime.fromisoformat(data["phase_entered"]) if data.get("phase_entered") else datetime.now(),
...
class LifecycleStateMachine:
    ...
    def compute(
        ...
        days_active = (datetime.now() - first_activity).days
        ...
        phase_entered = self._estimate_phase_entry(
            phase, sorted_activities, days_active
        )
        days_in_phase = (datetime.now() - phase_entered).days
        
        # Count activities in current phase
        activities_in_phase = len([
            a for a in activities 
            if a.timestamp >= phase_entered
        ])
    ...
    def _determine_phase(self, days: int, activities: int) -> LifecyclePhase:
        ...
        if days >= self.DAY_THRESHOLDS["veteran"] or activities >= self.ACTIVITY_THRESHOLDS["veteran"]:
            return "legend"
        if days >= self.DAY_THRESHOLDS["established"] or activities >= self.ACTIVITY_THRESHOLDS["established"]:
            return "veteran"
        if days >= self.DAY_THRESHOLDS["developing"] or activities >= self.ACTIVITY_THRESHOLDS["developing"]:
            return "established"
        if days >= self.DAY_THRESHOLDS["rookie"] or activities >= self.ACTIVITY_THRESHOLDS["rookie"]:
            return "developing"
        return "rookie"
    ...
    def _calculate_progress(
        ...
        day_progress = min(1.0, (days - prev_day_threshold) / max(1, day_threshold - prev_day_threshold))
        activity_progress = min(1.0, (activities - prev_activity_threshold) / max(1, activity_threshold - prev_activity_threshold))
