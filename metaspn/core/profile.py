...
class PlatformPresence:
    ...
    def is_rookie(self) -> bool:
        ...
        days_on_platform = (datetime.now() - self.joined_at).days
...
class UserProfile:
    ...
    def from_dict(cls, data: dict) -> "UserProfile":
        ...
        return cls(
            ...
            last_computed=datetime.fromisoformat(data["last_computed"]) if data.get("last_computed") else datetime.now(),
            platforms=[PlatformPresence.from_dict(p) for p in data.get("platforms", [])],
            lifecycle=LifecycleState.from_dict(data["lifecycle"]) if data.get("lifecycle") else None,
...
def compute_profile(
    ...
    from metaspn.repo.reader import load_minimal_state, load_activities
    from metaspn.repo.writer import cache_profile as write_cache, try_load_cached_profile
    ...
    from metaspn.core.level import LevelCalculator, RarityCalculator, AchievementSystem
    from metaspn.core.card import CardData
...
def _compute_platform_presences(activities: List[Activity]) -> List[PlatformPresence]:
    ...
    for platform, data in platform_data.items():
        ...
        presences.append(PlatformPresence(
            platform=platform,
            role=role,
...
def _compute_game_alignment(signature: GameSignature) -> float:
    ...
    scores = [signature.G1, signature.G2, signature.G3, 
              signature.G4, signature.G5, signature.G6]
    if not any(scores):
        return 0.0
